...
class VehicleUploadController extends Controller
{
    ...
    public function update(Request $request)
    {
        foreach ($request->order as $order => $id) {
            $position = Vehicle_photo::where('user_id', $this->user->id)->find($id);
    ...
    {
        $photo = Vehicle_photo::where('user_id', $this->user->id)->find($id);
        if ($photo->id){
        ...
        return response()->json(['error' => 'Imagen não encontrada.']);
